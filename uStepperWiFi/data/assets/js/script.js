...
var lastG1CommandCount = 0;		//number of joystick handlers executed since last move was send to arm
...
function joystickControl(){
	...
	if(zJoystickActive === false && xyJoystickActive === false)
	{
		...
		if( JSON.stringify(command) != JSON.stringify(lastG1Command) ||
			lastG1CommandCount >= 10)		//make sure that we STOP movement if the joystick is not active,
											//but the arm for some reason didn't pick up last stop command
		{
			lastG1Command = command;
			lastG1CommandCount = 0;
